...
class ChanceNode:
    ...
    def __init__(self, action: Tuple[str, Any], parent: 'DecisionNode'):
        ...
        self.natural_counts: Dict[YahtzeeState, int] = {}
...
def mcts_search(root_state: YahtzeeState,
                ...
                Cp: float = 40.0,
                k: float = 2.0,
                alpha: float = 0.5) -> Optional[Tuple[str, Any]]:
    """Return the best action from root_state using ExpectiMax MCTS with Progressive Widening."""
    ...
    for _ in range(iterations):
        ...
        while not node.state.is_terminal():
            if node.untried_moves:
                ...
                chance_node.natural_counts[next_state] = 1
                ...
                max_children = max(1, math.ceil(k * (best_chance_node.visits ** alpha)))
                if len(best_chance_node.children) < max_children:
                    # Allowed to discover new outcomes
                    ...
                    if next_state not in best_chance_node.children:
                        ...
                        best_chance_node.natural_counts[next_state] = 1
                        node = child_decision
                        break
                    else:
                        best_chance_node.natural_counts[next_state] += 1
                    ...
                    # PW restricted: sample from already discovered outcomes based on natural frequency
                    states = list(best_chance_node.children.keys())
                    weights = [best_chance_node.natural_counts[s] for s in states]
